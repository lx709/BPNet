...
def intersectionAndUnion(output, target, K, ignore_index=255):
    ...
    intersection = output[np.where(output == target)[0]]
    area_intersection, _ = np.histogram(intersection, bins=np.arange(K), range=(0,K-1))
    area_output, _ = np.histogram(output, bins=np.arange(K), range=(0,K-1))
    area_target, _ = np.histogram(target, bins=np.arange(K), range=(0,K-1))
    area_union = area_output + area_target - area_intersection
    return area_intersection, area_union, area_target
...
def intersectionAndUnionGPU(output, target, K, ignore_index=0):
    ...
    intersection = output[output == target]
    # https://github.com/pytorch/pytorch/issues/1382
    area_intersection = torch.histc(intersection.float().cpu(), bins=K-1, min=1, max=K - 1)
    area_output = torch.histc(output.float().cpu(), bins=K-1, min=1, max=K - 1)
    area_target = torch.histc(target.float().cpu(), bins=K-1, min=1, max=K - 1)
    area_union = area_output + area_target - area_intersection
    return area_intersection.cuda(), area_union.cuda(), area_target.cuda()
